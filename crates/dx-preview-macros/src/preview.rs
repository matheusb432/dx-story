...
pub(crate) fn expand(arguments: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    ...
    let mut errors = None;
    let arguments = diagnostics::capture(&mut errors, PreviewArguments::parse(arguments));
    ...
    let derived_id = diagnostics::capture(&mut errors, naming::preview_id(&function.sig.ident));
    let facade = diagnostics::capture(&mut errors, facade::path(function.sig.ident.span()));
    diagnostics::finish(errors)?;

    let arguments =
        diagnostics::validated(arguments, "preview arguments", function.sig.ident.span())?;
    let facade = diagnostics::validated(facade, "dx-preview facade", function.sig.ident.span())?;
    let derived_id =
        diagnostics::validated(derived_id, "derived preview ID", function.sig.ident.span())?;
    let preview_const_ident = naming::preview_const_ident(&function.sig.ident);
    let component_ident = naming::preview_component_ident(&function.sig.ident);
    let preview_id = arguments
        .metadata
        .id
        .unwrap_or_else(|| LitStr::new(&derived_id, function.sig.ident.span()));
    let preview_name = arguments.metadata.name.unwrap_or_else(|| {
        LitStr::new(
            &naming::preview_name(&function.sig.ident),
    ...
    Ok(quote! {
        ...
        const #preview_const_ident: &'static #facade::Preview =
            &#facade::__private::preview(
                #preview_id,
                #preview_name,
                #description,
...
impl PreviewArguments {
    ...
    fn push(&mut self, argument: Meta) {
        let Meta::NameValue(argument) = argument else {
            diagnostics::combine(
                ...
                syn::Error::new_spanned(
                    argument,
                    "preview arguments must use `key = value` syntax",
                ),
            );
            return;
        };
        if !self.metadata.push(&argument, &mut self.errors) {
            diagnostics::combine(
                &mut self.errors,
                syn::Error::new_spanned(argument.path, "unknown preview argument"),
            );
        }
    }

    fn validate(&mut self) {
        self.metadata.validate("preview", &mut self.errors);
    }
}
...
fn validate_signature(function: &ItemFn, errors: &mut Option<syn::Error>) {
    if function.sig.constness.is_some() {
        diagnostics::combine(
            errors,
            syn::Error::new(function.sig.span(), "preview functions must not be const"),
        );
    }
    if function.sig.asyncness.is_some() {
        diagnostics::combine(
            errors,
            syn::Error::new(function.sig.span(), "preview functions must not be async"),
        );
    }
    if !matches!(function.sig.safety, syn::Safety::Default) {
        diagnostics::combine(
            ...
            syn::Error::new(
                function.sig.span(),
                "preview functions must not use safety qualifiers",
            ),
        );
    }
    if function.sig.abi.is_some() {
        diagnostics::combine(
            ...
            syn::Error::new(
                function.sig.span(),
                "preview functions must use the Rust ABI",
            ),
        );
    }
    if !function.sig.inputs.is_empty() {
        diagnostics::combine(
            ...
            syn::Error::new_spanned(
                &function.sig.inputs,
                "preview functions must not accept arguments",
            ),
        );
    }
    if function.sig.variadic.is_some() {
        diagnostics::combine(
            ...
            syn::Error::new_spanned(
                ...
                "preview functions must not be variadic",
            ),
        );
    }
    if !function.sig.generics.params.is_empty() || function.sig.generics.where_clause.is_some() {
        diagnostics::combine(
            ...
            syn::Error::new_spanned(
                ...
                "preview functions must not be generic",
            ),
        );
    }
    if !returns_element(&function.sig.output) {
        diagnostics::combine(
            ...
            syn::Error::new_spanned(
                &function.sig.output,
                "preview functions must explicitly return `Element`",
            ),
        );
    }
}
